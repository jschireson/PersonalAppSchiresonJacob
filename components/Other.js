...
const Other = (props) => {
  ...
  const getData = async () => {
        try {
          // the '@profile3_info' can be any string
          const jsonValue = await AsyncStorage.getItem('@profile3_info')
          let data = null
          if (jsonValue!=null) {
  ...
  const storeData = async (value) => {
        try {
          ...
          await AsyncStorage.setItem('@profile3_info', jsonValue)
  ...
  // we could be more selective and only remove the profile3_info, but for
  ...
  const clearAll = async () => {
      ...
      <Pressable
        ...
        {({ pressed }) => (
          <Text style={styles.header}>
            {pressed ? note3 : "First Pet's Name"}
          </Text>
        )}
      </Pressable>
              {/* <Text style={styles.header}>
